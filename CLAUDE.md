...
- **`ImageCarousel` island** is the one shared photo carousel (crossfade, auto-advance every **2000 ms**, pauses on hover/focus, reduced-motion-safe, dot tablist). Props: `slides` (build-optimized `{src, srcset, alt}` from `getImage`), `label`, `intervalMs`, `aspectClass`, `sizes`, `objectClass`. Used by **HeadSpa** (4 photos, default `aspect-[4/3]`) and **Story** (2 treatment-room photos at `aspect-[3/2]` — the 16:9 originals center-crop cleanly there). `TestimonialCarousel` also auto-advances every 2000 ms; keep all carousel intervals at 2 s (owner's setting).
...
- `src/assets/images/couples-room.jpg` ← `a58ec307…` (couples room w/ sage linens — Location; original was a 29MB PNG, downscaled)
- `src/assets/images/sanctuary-{1,2}.jpg` ← owner-supplied DSLR photos (`_DSC9452`, `_DSC9461`, Mar 2025): couples suite (two tables) + single treatment room. 16:9 originals downscaled to 1800px wide; they are the two Story carousel slides.
