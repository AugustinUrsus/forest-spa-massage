...
`Header` → `Hero` (`#top`) → `Story` (`#story`) → `Services` (`#services`) → `Packages` (`#packages`) → `MonthlySpecial` (`#specials`) → `HeadSpa` (`#head-spa`) → `Membership` (`#membership`) → `Testimonials` → `GiftCard` (`#gift-cards`) → `Location` (`#location`) → `Footer` → `StickyMobileBar`.
- All section components live in `src/components/sections/*.astro`; React islands in `src/components/react/*.tsx` (`ServiceTabs`, `PackageSelector`, `TestimonialCarousel`, `MobileNav`, `ImageCarousel`). `navLinks` in `siteData.ts` drives header + drawer and must stay in sync with the section ids above.
- **Section background rhythm** alternates light/dark: Hero (charcoal) → Story (`bg-sand`) → Services (`bg-sand-deep`) → Packages (`bg-cloud`) → MonthlySpecial (charcoal) → HeadSpa (`bg-cloud`) → Membership (charcoal) → Testimonials (`bg-charcoal-deep`, a shade darker so the three closing dark bands stay distinct) → GiftCard (charcoal) → Location (`bg-cloud`). Hero's bottom fade (`to-sand`) must match whatever section follows it.
...
- **`ImageCarousel` island** is the one shared photo carousel (crossfade, auto-advance every **2000 ms**, pauses on hover/focus, reduced-motion-safe, dot tablist). Props: `slides` (build-optimized `{src, srcset, alt}` from `getImage`), `label`, `intervalMs`, `aspectClass`, `sizes`, `objectClass`. Used by **HeadSpa** (4 photos, default `aspect-[4/3]`) and **Story** (3 treatment-room photos at `aspect-[3/2]` — the original 4:3 couples-room shot leads, then the two 16:9 photos, all center-cropping cleanly there). `TestimonialCarousel` also auto-advances every 2000 ms; keep all carousel intervals at 2 s (owner's setting).
- **HeadSpa** renders the intro/benefits column + the `ImageCarousel` island, then a 2-up card grid from `headSpa.services` (each card books via `bookingUrl(s.id)`). The big "Book Head Spa" CTA uses `booking.headSpa`.
...
- `monthlySpecial` + `standingOffers` — **update `monthlySpecial` every month** (`month`, `headline`, `name` + `shortName` for the hero ribbon, the `options[]` price tiers `{duration, price, value}`, `validity`, `summary`, `inclusions`, and the Fresha `url`). Multiple duration tiers are supported: `MonthlySpecial.astro` renders one price row per entry in `options` (each with its own struck-through value + "Save $X" badge), and the hero ribbon shows `shortName` + "from $" of `options[0]`. Some specials have no `offerItemId`, so this object carries a full prebuilt Fresha `url` instead of an `id`. Rendered by `MonthlySpecial.astro` (`#specials`) + the hero ribbon. **August 2026:** Summer Cooling Head Spa + Foot Massage Combo — 90 min $110 (value $125) / 120 min $130 (value $145), valid Aug 1–31. **September 2026 (current):** Labor Day Reset: 90-Min Full Body & TMJ Relief Combo — 90 min $125 (value $175), valid Sep 1–30; single tier, so the hero ribbon drops the "from " prefix (`Hero.astro` renders it only when `options.length > 1`).
...
- `src/assets/images/couples-room.jpg` ← `a58ec307…` (couples room w/ sage linens — first Story carousel slide + Location; original was a 29MB PNG, downscaled)
- `src/assets/images/sanctuary-{1,2}.jpg` ← owner-supplied DSLR photos (`_DSC9452`, `_DSC9461`, Mar 2025): couples suite (two tables) + single treatment room. 16:9 originals downscaled to 1800px wide; they are Story carousel slides 2–3.
